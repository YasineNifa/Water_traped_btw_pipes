	...
	def draw_water_1(self):
		...
		while index != length-1:
			...
			for j in range(index+1,length):
				if self.pipes[j] >= pipe:
					bo = True
					break

			if bo : # Si on trouve un element sup ou égale à pipe
				for k in range(index+1,j):
					#area +=pipe-self.pipes[k]
					pygame.draw.rect(self.win, self.BLUE, (k * self.inc_x, self.h - (self.inc_y * min(pipe, self.pipes[index])), self.inc_x, self.h))
			else : # sinon
				maxe = max(self.pipes[index+1:])
				for k in range(index+1,length):
					if self.pipes[k] == maxe:
						j = k
						pipe = self.pipes[j]
						break
				for k in range(index+1,j):
...
instance = Visual([9,1,2,9,5,4,7],255,255)
instance.show()